...
def action_wait():
    ...
    if actionComplete == False:
        ...
        with eventlet.Timeout(20,False):
            while len(action_list) != 0 or actionComplete == False:     # 当动作列表不为0 或 当前动作未完成
                time.sleep(0.001)
...
def     action_append(act_name):
    ...
    m = hashlib.md5()
    name_encode = bytes(act_name,encoding='utf8')   # 给新添加进来的动作随机生成一个Hash数并存储
...
if __name__ == '__main__':
    ...
    while True:
        if len(action_list) == 0:
            act_name = input("please act_name123:")
            print("haha:",act_name)
            if act_name != None:
                print("act_name")
                action_append(act_name)
        else:
            print("队列有未执行动作 length：", len(action_list))
            time.sleep(1)
