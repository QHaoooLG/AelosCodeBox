...
def follow_lines(cmd_ctrl):
    ...
    while True :
        ...
        dot1=np.array([[207,120]])
        # dot2=np.array([[207,300]])
        for i in range(len(contours)):
            if cv2.contourArea(contours[i])>3000:
                for point in contours[i]:
                    if point[0][1] == 120:
                        dot1 = np.vstack((dot1, point))
                    # if point[0][1] == 300:
                    #     dot2 = np.vstack((dot2, point))
        ...
        x = []
        for idx in range(len(dot1)-1):
            x.append(dot1[idx+1][0])
        x.sort()
        print(x)
        print(len(x))

        # _x = []
        # for idx in range(len(dot2)-1):
        #     _x.append(dot2[idx+1][0])
        # _x.sort()
        # print(_x)
        # print(len(_x))
        ...
        if len(x)>2:
            ...
            # _x_max=0
            # _x_min=0
            # for n in range(len(_x)//2):
            #     _x_min = _x[n] + _x_min
            #     _x_max = _x[-(n+1)] + _x_max
            # _x_max = x_max/(len(_x)//2)
            # _x_min = x_min/(len(_x)//2)
        ...
        if dot1[0][0]>x_max: # or dot2[0][0]>_x_max:
            action("turn005L")
            print("左转5度")
        elif dot1[0][0]<x_min: # or dot2[0][0]<_x_min:
            action("turn005R")
            print("右转5度")
        else:
            # action("AverageWalk_01")
            action("fastForward01")
            print("走两步")
